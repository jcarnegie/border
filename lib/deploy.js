...
let createIntegration = r.curry(async (apiSpec, methodSpec, resourceId, functionName) => {
    let params = methodRequestParams(apiSpec, methodSpec);
...
let bindEndpointAndFunction = r.curry(async (logFn, apiSpec, methodSpec) => {
    ...
    let createEndpoint = r.composeP(
        () => gateway.createIntegrationResponse(apiSpec.region, apiSpec.api.id, resourceId, methodSpec.method, 200),
        () => gateway.createMethodResponse(apiSpec.region, apiSpec.api.id, resourceId, methodSpec.method, 200),
        () => createIntegration(apiSpec, methodSpec, resourceId, functionName),
...
let mapAsyncSerial = async (fn, list) => {
    let result = await fn(r.head(list));
    let tail = r.tail(list);
    let results = [];

    if (tail.length > 0) {
        results = await mapAsyncSerial(fn, tail);
...
let go = r.curry(async (logFn, region, env, stage, dest, spec) => {
    try {
        ...
        await mapAsyncSerial(installFunctionModules(dest, stage), apiSpec.endpoints);
        logFn("ok", "installed lambda function NPM modules");

        // Make sure all gateway endpoints are created
        await * r.map(updateMethod(region, api.id, apiSpec.resources, spec), apiSpec.endpoints);
