...
let go = async (region, env, stage, spec, dest) => {
    ...
    let processResource = r.curry(async (accountId, api, resources, gateway, region, env, apiName, stage, dest, spec) => {
        let resource = null;
        let method = null;
        let lambdaZip = null;
        let resp = null;
    ...
    await * r.map(processResource(accountId, api, resources, gateway, region, env, apiName, stage, dest), endpoints);
