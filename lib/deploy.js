...
let createGatewayLambdaFunction = r.curry(async (apiSpec, handler, name, zip) => {
...
let addMethodPermission = r.curry(async (apiSpec, methodSpec, name) => {
...
let createIntegration = r.curry(async (apiSpec, methodSpec, resourceId, credentials, functionArn) => {
...
let bindEndpointAndFunction = r.curry(async (apiSpec, methodSpec) => {
    ...
    lambdaZip      = await zip(apiSpec, methodSpec, functionName);
    ...
    await addMethodPermission(apiSpec, methodSpec, functionName);
    await createIntegration(apiSpec, methodSpec, resId, null, funcArn);
});
...
let go = r.curry(async (region, env, stage, dest, spec) => {
    try {
        ...
        await * r.map(bindEndpointAndFunction(apiSpec), endpoints);
